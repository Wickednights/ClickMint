...
Step-by-step checklist: **Base Sepolia first** for the full game loop (minute rounds, POT, Block Bet, vesting), then **Base mainnet** only where real ETH is required — typically **Uniswap v2–style LP** and optional staged testing on **preview / Vercel Preview** envs. Complete each phase before moving on; pause to update env, ledgers, and docs after deploys.
...
## Phase 2 — Deploy / verify on Base Sepolia (primary testnet)

1. **Open terminal** at `<repo-root>`, then go to contracts:
...
5. **Save output** — All contract addresses (CLICK, game, treasury, trophy, escrow). The legacy **SecretPrizeWallet** path is **not** part of the current `ClickMintGame` constructor.
...
7. **Run post-deploy checks** — e.g. `npm run verify:base-sepolia` or `npx hardhat run scripts/verify-deployment.ts --network baseSepolia` with any env the script expects (`GAME_ADDRESS`, `CLICK_ADDRESS`, `EXPECTED_MAX_SUPPLY_WEI` for testnet cap, etc.).
8. **Set `potKeeper` (optional on Sepolia)** — If you use Vercel cron on Sepolia: run [`scripts/set-pot-keeper.ts`](../contracts/scripts/set-pot-keeper.ts) so on-chain **`potKeeper`** equals the address that signs with **`POT_KEEPER_PRIVATE_KEY`**. The cron route calls **`finalizeRound`** (minute cadence).
...
7. **Smoke the app** — Connect wallet on Base Sepolia, deposit, click, Block Bet, POT UI / history.
...
3. **Cron (optional)** — Set **`CRON_SECRET`** and **`POT_KEEPER_PRIVATE_KEY`** for a wallet funded on **84532**; ensure **`potKeeper`** on the game matches that address. Route: **`GET /api/cron/finalize-round`** (see `frontend/vercel.json`).
4. **Deploy** — Push or trigger deploy; confirm the site loads and the wallet targets **Base Sepolia**.

---

## Preview / staging (mainnet chain without “full prod” assumptions)

- Use **Vercel Preview** or **Development** environment variables with **`NEXT_PUBLIC_CHAIN_ID=8453`** when you need to exercise **mainnet RPCs** or **LP-related flows** without treating the deployment as canonical production.
- A hostname such as **preview.clickmint.app** (or your team’s preview URL) can be used for staged experiments; keep **`CRON_SECRET`** / keeper keys separate from production so automation never hits the wrong contracts.
...
2. **Fund a dedicated keeper** — For **`finalizeRound`** gas on **8453** (can be the same as deployer only if you accept the risk).
...
7. **`verify-deployment.ts` on `base`** — `npm run verify:base` with mainnet expectations (e.g. **10B** CLICK cap per [GAME_MECHANICS.md](GAME_MECHANICS.md) — see [POST_DEPLOY_VERIFICATION.md](POST_DEPLOY_VERIFICATION.md)).
...
1. **Smoke** — Deposit, click, vesting, POT + Block Bet on **8453** with **small** amounts (or use Preview env first).
2. **LP** — Follow [LP_AERODROME_AND_AUTOMATION.md](LP_AERODROME_AND_AUTOMATION.md) for **Uniswap v2–style** pool/router addresses; v3/v4 are out of scope for “simple LP” unless you extend the playbook. Size liquidity conservatively at first.
