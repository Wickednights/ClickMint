...
- **Parimutuel pool per round:** **20%** deposit slice for that round **plus** all ETH explicitly staked via **`placeBet(slot)`** (payable) on slots `0–3` for the current round.
- **Settlement:** On `finalizeRound`, a **winning slot** is drawn (same entropy stream as POT logic allows). Stakers on that slot split the **entire** Block Bet pot for that round **proportionally** to their stake on the winning slot.
- **No bets on the winning slot:** Per product spec, the pot **carries** forward (`blockBetCarry`) until a future round pays out.
